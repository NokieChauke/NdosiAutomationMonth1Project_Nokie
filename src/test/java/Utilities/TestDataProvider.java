...
public class TestDataProvider {
    ...
    public static Object[][] getInventoryData() {
        ...
        Object[][] testData = new Object[data.size()][8];
        for (int i = 0; i < data.size(); i++) {
            Map<String, String> row = data.get(i);
            testData[i][0] = row.get("DeviceType");
            testData[i][1] = row.get("Brand");
            testData[i][2] = row.get("UnitPrice");
            testData[i][3] = row.get("Color");
            testData[i][4] = row.get("ColorValue");
            testData[i][5] = row.get("Quantity");
            testData[i][6] = row.get("Subtotal");
            testData[i][7] = row.get("Address");
        }
        return testData;
    }
}
